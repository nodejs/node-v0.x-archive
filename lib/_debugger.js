...
var helpMessage = [
    'Commands:'
  , '  cmd:p[rint] arg:expr - inspect the given arg:expr'
  , '  cmd:s[tep] arg:n - step in arg:n times'
  , '  cmd:n[ext] arg:n - step next arg:n times'
  , '  cmd:c[ontinue] - continue execution'
  , '  cmd:b[t|acktrace] - print the current execution state stacktrace'
  , '  cmd:scripts - list the active scripts'
  , '  cmd:version - print the V8 version'
  , '  cmd:q[uit]'
].join('\n')
 .replace(/cmd:(\S+)/g, '\x1b[33m$1\x1b[0m')
 .replace(/arg:(\S+)/g, '\x1b[90m$1\x1b[0m');
...
function startInterface() {
  ...
  term.on('line', function executeCommand(cmd) {
    if (cmd == 'quit' || cmd == 'q' || cmd == 'exit') {
      tryQuit();
    } else if (/^h(elp)?/.test(cmd)) {
    ...
    } else if (/^n(ext)?( +\d+)?/.test(cmd)) {
      var n = parseInt(RegExp.$2 || '1', 10);
      c.step('next', n, function (res) {
        // Wait for break point. (disable raw mode?)
      });

    } else if (/^s(tep)?( +\d+)?/.test(cmd)) {
      var n = parseInt(RegExp.$2 || '1', 10);
      c.step('in', n, function (res) {
        // Wait for break point. (disable raw mode?)
      });
      ...
      if (/^\s*$/.test(cmd)) {
        ...
        error('unknown command "%s". Try "help"', cmd);
      }
      term.prompt();
    }
