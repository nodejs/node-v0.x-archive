...
function onclienthello(hello) {
  ...
  function callback(err, session) {
    ...
    // NOTE: That we've OpenSSL's internal session storage disabled in
    ...
    if (ret && ret.servername) {
      ...
      self._SNICallback(hello.servername.toString(), onSNIResult);
    } else {
      self.ssl.endParser();
    }
  }
  ...
  if (hello.sessionId.length <= 0 ||
    ...
    if (hello.servername && this._SNICallback)
      this._SNICallback(hello.servername.toString(), onSNIResult);
    else
      this.ssl.endParser();
  }
}
...
Server.prototype.SNICallback = function(servername, callback) {
  var ctx;
