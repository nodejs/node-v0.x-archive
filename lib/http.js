...
// always joined.
IncomingMessage.prototype._addHeaderLine = function (rawField, value) {
  this.rawHeaders += rawField + ': ' + value + '\r\n';

  var field = rawField.toLowerCase();
  switch (field) {
    // Array headers:
    case 'set-cookie':
...
OutgoingMessage.prototype._storeHeader = function (firstLine, headers) {
  ...
  if (typeof headers == 'string' && headers.match(rawHeadersExpression)) {
    rawHeaders = headers;
  } else if (this.rawHeaders) {
  ...
  if (rawHeaders) {
    var lines = rawHeaders.split(CRLF);
    for (var i = 0, l = lines.length; i < l; i++) {
      var line = lines[i];
      if (line.length > 0) {
        var matches = line.match(headerExpression);
        if (matches) {
          store(matches[1], matches[2]);
        }
      }
    }
...
ServerResponse.prototype.writeHead = function (statusCode) {
  var reasonPhrase, potentialHeaders, headers;

  if (typeof arguments[1] == 'string' &&
      !arguments[1].match(rawHeadersExpression)) {
    reasonPhrase = arguments[1];
    potentialHeaders = arguments[2];
  } else {
    reasonPhrase = STATUS_CODES[statusCode] || "unknown";
    potentialHeaders = arguments[1];
  }

  if (typeof potentialHeaders == 'object' ||
      (typeof potentialHeaders == 'string' &&
      potentialHeaders.match(rawHeadersExpression))) {
...
Client.prototype.request = function (method, url, headers) {
  if (typeof url != "string" || url.match(rawHeadersExpression)) {
