...
exports.createConnection = function (port, host, opts) {
  var s = new Stream();
  s.connect(port, host, opts);
  return s;
};
...
Resolver = function(to_resolv, fn) { 
  var cnt = 0;
  var todo = [];
  for(var i in to_resolv) { 
    ...
    dns.lookup(to_resolv[i].host, function(i) { 
      return function(err, ip, addrType) {
        ...
        var ret = true;
        for(var cnt = 0; cnt <= i && todo[cnt]; ++cnt) {
          if (ret) {
      ...
      to_resolv.push({ 
        ...
        completed: function(err, ip, addressType) { 
          if (err) {
            ...
            try {
              bind(self.fd, parseInt(bind_opts.port, 10), ip)
            } catch (e) {
              self.destroy(e);
              return false;
            }
