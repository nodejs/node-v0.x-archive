...
var dtls_wrap = process.binding('tls_wrap');

var debug = util.debuglog('dtls');
...
function onclienthello(hello) {
  ...
  function callback(err, session) {
    if (onceSession)
      return self.destroy(new Error('DTLS session callback was called 2 times'));
    onceSession = true;
    ...
    // NOTE: DTLS Session ticket doesn't include servername information
    //
  ...
  function onSNIResult(err, context) {
    if (onceSNI)
      return self.destroy(new Error('DTLS SNI callback was called 2 times'));
    onceSNI = true;
...
function DTLSSocket(options, listener) {

  dgram.Socket.call(this, { type: 'udp4' }, listener);
  ...
  if (!this._handle)
    this.once('bind', this._init.bind(this));
  else
    this._init();
}
util.inherits(DTLSSocket, dgram.Socket);
exports.DTLSSocket = DTLSSocket;

DTLSSocket.prototype._init = function () {    
  ...
  this.ssl = dtls_wrap.wrap(this._handle, credentials.context, options.isServer);
...
DTLSSocket.prototype._tlsError = function(err) {
  this.emit('_tlsError', err);
  if (this._controlReleased)
    this.emit('error', err);
};

DTLSSocket.prototype._releaseControl = function() {
  if (this._controlReleased)
    return;
...
DTLSSocket.prototype._finishInit = function() {
...
DTLSSocket.prototype._start = function() {
  this.ssl.start();
};

DTLSSocket.prototype.setServername = function(name) {
  this.ssl.setServername(name);
};

DTLSSocket.prototype.setSession = function(session) {
  if (util.isString(session))
    session = new Buffer(session, 'binary');
  this.ssl.setSession(session);
};

DTLSSocket.prototype.getPeerCertificate = function() {
  if (this.ssl) {
...
DTLSSocket.prototype.getSession = function() {
  if (this.ssl) {
...
DTLSSocket.prototype.isSessionReused = function() {
  if (this.ssl) {
...
DTLSSocket.prototype.getCipher = function(err) {
  if (this.ssl) {
...
function Server(/* [options], listener */) {
  ...
  debug('before : dgram.Server.call');

    // Now attach the UDP socket to the DTLS socket
  this.socket = new DTLSSocket({
    ...
    function listener() {
        this.socket._tlsError(new Error('TLS handshake timeout'));
    }

    // TODO: Look at this closer
    // DGRAM - Not like an accept, DGRAM must wait the first packet...
    //if (timeout > 0) {
    //      socket.setTimeout(timeout, listener);
    //}
    this.socket.once('secure', function () {
        //self.socket.setTimeout(0, listener);
        ...
        if (!self.socket.destroyed) {
            self.socket._releaseControl();
            self.emit('secureConnection', self.socket);
        }
    });

    this.socket.on('_tlsError', function (err) {
    ...
    // constructor call
    dgram.Server.call(this, function () {
      debug('after : dgram.Server.call');
    });
}


util.inherits(Server, dgram.Server);
exports.Server = Server;

exports.Server = function() {
   debug('exports.Server');
  return new Server(arguments[0], arguments[1]);
};

exports.createServer = function(options, listener) {
    debug('exports.createServer');
...
function normalizeConnectArgs(listArgs) {
  ...
  var args = undefined; //net._normalizeConnectArgs(listArgs);
  var options = args[0];
  var cb = args[1];
...
exports.connect = function(/* [port, host], options, cb */) {
  ...
  var socket = new DTLSSocket(options.socket, {
