...
class Socket FINAL {
 public:
  ...
  bool Bind(int port) WARN_UNUSED_RESULT;
  bool Listen(int backlog) WARN_UNUSED_RESULT;
  Socket* Accept() WARN_UNUSED_RESULT;

  // Client initialization.
  bool Connect(const char* host, const char* port) WARN_UNUSED_RESULT;
  ...
  // Data Transimission
  // Return 0 on failure.
  int Send(const char* buffer, int length) WARN_UNUSED_RESULT;
  int Receive(char* buffer, int length) WARN_UNUSED_RESULT;
