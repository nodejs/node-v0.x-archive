...
defaults to `'utf8'. The `start` and `end` parameters default to `0` and `buffer.length` when 
`undefined` or passed as `NaN`.
...
if the target memory region overlaps with the source. If `undefined` or passed 
as `NaN`, the `targetStart` and `sourceStart` parameters default 
