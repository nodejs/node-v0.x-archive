...
was called. Stack traces are subject to [V8's stack trace API](https://code.google.com/p/v8/wiki/JavaScriptStackTraceApi). Stack traces only
extend as far back as the current event loop tick, *or* a number of frames given by
...
method name), but occasionally it will not be able to find a suitable name. If it
cannot, it simply displays location information for that frame. If it
can, it will display the name, and put the location information in
parentheses.
